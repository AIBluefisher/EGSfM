    ...
    while (!qu.empty()) {
        ...
        if (cur_id == dst) { // arrive destination
            ...
            while (parents[id] != -1) {
                ...
                std::cout << parents[id] << "->";
                // #endif
                // T2 edge = this->GetEdge(parents[id], id);
                T2 edge = edges[parents[id]][id];
...
void Graph<T1, T2>::ShowInfo() const
{
    ...
    log::Logger::LogToScreen("[Graph Info]\n", LogType::HEAD);
    log::Logger::LogToScreen("Total nodes: " + std::to_string(GetNodesNum()), LogType::INFO);
    log::Logger::LogToScreen("\nTotal edges: " + std::to_string(GetEdgesNum()), LogType::INFO);
    log::Logger::LogToScreen("\n[Node]: \n", LogType::INFO);
