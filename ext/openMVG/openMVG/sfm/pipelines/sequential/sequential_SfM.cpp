...
bool SequentialSfMReconstructionEngine::Process() {
  ...
  // if (!tracksBuilder.LoadFromFile(matches_dir_ + "/tracks.txt", map_tracks_)) {
  //   if (!InitLandmarkTracks())
  //     return false;
  // } else {
  ...
  if (initial_pair_ == Pair(0,0))
  {
    ...
    {
      // std::cout << "end automatic initialpairchoice\n";
...
bool SequentialSfMReconstructionEngine::ChooseInitialPair(Pair & initialPairIndex)
{
  // std::cout << "begin chooseInitialPair" << std::endl;
  if (initial_pair_ != Pair(0,0))
  {
...
bool SequentialSfMReconstructionEngine::AutomaticInitialPairChoice(Pair & initial_pair)
{
...
#endif
    {
      ...
      if (valid_views.count(I) && valid_views.count(J))
      {
        ...
        if (cam_I && cam_J)
        {
          ...
          {
            // update matches via inliers
            // std::cout << "begin UpdateMatches\n";
            UpdateMatches(I, J, relativePose_info.vec_inliers);
            // std::cout << "end UpdateMatches\n";
            ...
            // std::cout << relativePose_info.vec_inliers.size() << std::endl;
  ...
  // std::cout << "end automatic initial pair selection";
  ...
  if (bRefine_using_BA)
  {
    ...
    for (const auto & landmark_entry : tiny_scene.GetLandmarks())
    {
          ...
          residual_J.norm() < relativePose_info.found_residual_precision)
      {
        ...
        const IndexT id_featI = ob_xI.id_feat;
        const IndexT id_featJ = ob_xJ.id_feat;
        inlier_matches.push_back(IndMatch(id_featI, id_featJ));
      }
    }

    // UpdateMatches(I, J, inlier_matches);
      ...
      else
      {
        ...
        for (const std::pair<IndexT, IndexT>& trackViewIt : allViews_of_track)
        {
          ...
          if (J != I && valid_views.count(J) != 0)
          {
            ...
            {
              IndexT feat_id_i = track.at(I);
              IndexT feat_id_j = allViews_of_track.at(J);
              ...
              const Vec2 xJ = features_provider_->feats_per_view.at(J)[feat_id_j].coords().cast<double>();

              // Position of the point in view I
              const Vec2 xI = features_provider_->feats_per_view.at(I)[feat_id_i].coords().cast<double>();
                 ...
                 )
              {
                ...
                // update matches' id
                Pair pair(I, J);
                // matches_[pair].push_back(IndMatch(feat_id_i, feat_id_j));
              } // 3D point is valid
              else
              {
...
void SequentialSfMReconstructionEngine::UpdateMatches(uint32_t i, uint32_t j, 
                                                      const std::vector<uint32_t>& inliers)
{
  Pair pair(i, j);
  PairWiseMatches& matches = matches_provider_->pairWise_matches_;
  IndMatches& ind_matches = matches[pair];
  IndMatches inlier_matches;
  for (auto inlier_index : inliers) {
    inlier_matches.push_back(ind_matches[inlier_index]);
  }
  // ind_matches.clear();
  // ind_matches.assign(inlier_matches.begin(), inlier_matches.end());
  UpdateMatches(i, j, inlier_matches);
}

void SequentialSfMReconstructionEngine::UpdateMatches(uint32_t i, uint32_t j, 
                                                      const matching::IndMatches& inlier_matches)
{
  Pair pair(i, j);
  // PairWiseMatches& matches = matches_provider_->pairWise_matches_;
  // IndMatches& ind_matches = matches[pair];
  // IndMatches tmp_matches;

  // ind_matches.clear();
  // ind_matches.assign(inlier_matches.begin(), inlier_matches.end());
  // for (auto inlier_match : inlier_matches) {
  //   // ind_matches.push_back(IndMatch(inlier_match.i_, inlier_match.j_));
  //   matches_[pair].push_back(IndMatch(inlier_match.i_, inlier_match.j_));
  // }
  // matches_provider_->pairWise_matches_[pair] = inlier_matches;
  matches_[pair].assign(inlier_matches.begin(), inlier_matches.end());
  
  // tmp_matches.assign(inlier_matches.begin(), inlier_matches.end());
  // matches_[pair] = tmp_matches;
}

void SequentialSfMReconstructionEngine::UpdateGlobalMatches()
{
  matches_provider_->pairWise_matches_ = matches_;
}
