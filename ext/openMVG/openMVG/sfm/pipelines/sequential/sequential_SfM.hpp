...
public:
  ...
  void SetMatchesDir(const std::string& dir)
  {
    matches_dir_ = dir;
  }

  openMVG::matching::PairWiseMatches GetMatches() const
  {
    return matches_;
    // return matches_provider_->pairWise_matches_;
  }

  void UpdateGlobalMatches();

  /// Initialize tracks
  bool InitLandmarkTracks();

  /// Select a candidate initial pair
  bool ChooseInitialPair(Pair & initialPairIndex);
...
private:
  ...
  void UpdateMatches(uint32_t i, uint32_t j, const std::vector<uint32_t>& inliers);
  void UpdateMatches(uint32_t i, uint32_t j, const matching::IndMatches& inlier_matches);
  ...
  openMVG::matching::PairWiseMatches matches_;
