...
struct TracksBuilder
{
  ...
  bool LoadFromFile(const std::string& filename, STLMAPTracks& map_tracks) 
  {
    ...
    size_t track_id, real_track_id = 0;
    size_t size;
    while (stream >> track_id >> size) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        sub_track.insert(std::make_pair(image_id, feature_id));
      }
      map_tracks.insert(std::make_pair(track_id, sub_track));
      // map_tracks.insert(std::make_pair(real_track_id++, sub_track));
    }
