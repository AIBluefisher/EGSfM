 ...
 ***********************************************************************/
int readClustering(char *filename, int *part, int nvtex){
  FILE *fpin;
  int i, numClus=0;
...
/*************************************************************************
* This function writes the coarsest matrix in Metis format
**************************************************************************/
void WriteCoarsestGraph(GraphType *graph, char *filename, int *wgtflag)
{
  ...
  for (i=0; i<nvtxs; i++){
    for (j=xadj[i]; j<xadj[i+1]; j++)
      fprintf(fpout, "%d %d ", adjncy[j]+1, adjwgt[j]); //index starts from 1 
    fprintf(fpout, "\n");
  }
...
/*************************************************************************
* This function reads the initial clustering of the coarsest graph
**************************************************************************/
  ...
  */
  ...
  for (xadj[0]=0, k=0, i=0; i<graph->nvtxs; i++) {
    ...
    for (;;) {
      ...
      if (readew) {
        ewgt = (idxtype)strtol(oldstr, &newstr, 10);
        oldstr = newstr;
      }
...
void WritePartition(char *fname, idxtype *part, int n, int nparts)
{
  ...
  sprintf(filename,"%s.part.%d", extracted, nparts);
